...
type UseSendSubmitOptions = {
  ...
  onStatusChange: (status: UploadStatusType) => void
...
const PRESIGN_STATUS_MAP: Record<string, UploadStatusType> = {
  idle: 'idle',
...
export function useSendSubmit(options: UseSendSubmitOptions) {
  const { uploadFile: presignUploadFile, reset: resetPresignUpload, presignStatus } = usePresignedUpload({
  ...
  watch(presignStatus, (newStatus) => {
    onStatusChange(PRESIGN_STATUS_MAP[newStatus] || 'idle')
  })
