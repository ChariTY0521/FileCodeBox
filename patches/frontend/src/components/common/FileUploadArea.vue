<template>
  ...
  >
    ...
    <div v-if="isUploading" class="text-center">
      <span :class="[isDarkMode ? 'text-indigo-400' : 'text-indigo-600', 'text-2xl font-bold']">{{ progress }}%</span>
    </div>

    <!-- 字节数（百分比下方居中） -->
    <div v-if="isUploading && showProgressDetails" class="text-center text-xs" :class="[isDarkMode ? 'text-gray-400' : 'text-gray-500']">
    ...
    <component
      :is="statusIcon"
      :class="['transition-colors duration-300', isUploading || isSuccess || hasError ? 'w-5 h-5' : 'w-16 h-16', statusIconClass]"
    />
    ...
    <p
      :class="[
        ...
        isUploading || isSuccess || hasError ? 'mt-2' : 'mt-4',
        isDarkMode
